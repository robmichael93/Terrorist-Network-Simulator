...
public class LoadGamePanel extends RPanel
{
  MainFrame mf;
  ...
  public LoadGamePanel(MainFrame mf)
  {
    this.mf = mf;
    setLayout(null);
    setBackground(Ggui.transparent); // for shadow border

    JLabel header = new JLabel("Select a previously saved game and then proceed");
...
rightDescriptions.setBackground(Ggui.transparent);
    ...
    proceedButt = ButtonFactory.make(ButtonFactory.LOADGAME_PROCEED,mf);
    proceedButt.setEnabled(false);
    add(proceedButt);
    JButton cancelButt = ButtonFactory.make(ButtonFactory.LOADGAME_CANCEL,mf);
    add(cancelButt);

    JLabel backLabel = new JLabel(Ggui.imgIconGet("LOADGAMEBACK"));
  ...
  public SavedGame getLoadedGame()
  {
    try
    {
      ObjectInputStream ois = new ObjectInputStream(new FileInputStream("./save/"+chosenName+".sgm"));
      SavedGame sg = (SavedGame)ois.readObject();
      return sg;
    }
    catch (Exception e)
    {
      System.out.println("bad game load");
     }
    return null;
  }
  public void done()
