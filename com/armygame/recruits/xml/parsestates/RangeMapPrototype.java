package com.armygame.recruits.xml.parsestates;

/**
 * Title: RangeMapPrototype
 * Description: Auto generated concrete <code>DocumentParseState</code>
 * Copyright:    Copyright (c) 2001
 * Company:
 * @author Neal Elzenga (Auto Generated)
 * @version 1.0
 */
import org.xml.sax.Attributes;
import com.armygame.recruits.utils.RangeMap;
import com.armygame.recruits.globals.MasterRangeMapIndex;
import com.armygame.recruits.xml.ParserStateMachine;
import com.armygame.recruits.xml.DocumentParseState;
import com.armygame.recruits.xml.ObjectArgument;
import com.armygame.recruits.xml.ConstructionMethodCall;
import com.armygame.recruits.xml.EndMethodCallSentinel;



/**
 * Class for handling XML RangeMapPrototype tags.  This class is a <b>Singleton</b>
 * (see 'Design Patterns' - Gamma et. al)
 * <b>IMPORTANT NOTE:</b> This class was auto generated by the
 * <b>MakeStates</b> tool, and requires user customization of the
 * following methods:
 * <code>InitializeState()</code>, <code>StartElement()</code>,
 * <code>EndElement</code>, <code>Characters</code>
 */
public class RangeMapPrototype extends DocumentParseState {

  /**
   * The single RangeMapPrototype
   */
  private static RangeMapPrototype theirInstance = null;

  /**
   * The constructor is <code>private</code> to enforce <b>Singleton</b>
   * behavior
   */
  private RangeMapPrototype() {
  }

  /**
   * Return the RangeMapPrototype <b>Singleton</b>
   * @return The RangeMapPrototype <b>Singleton</b>
   */
  public static RangeMapPrototype Instance() {
    if( theirInstance == null ) {
      theirInstance = new RangeMapPrototype();
    }
    return theirInstance;
  }


  /**
   * This state is called when we get the end element for state RangeMapPrototype
   * @param context The <code>ParserStateMachine</code> that is managing our parsing
   * @param uri
   * @param localName
   * @param qName
   * @see org.xml.SAX#endElement()
   */
  public void EndElement( ParserStateMachine context, String uri, String localName, String qName ) {
    context.RememberTagValue( "RangeMapPrototype" );
    context.AddConstructionTask( new ConstructionMethodCall( "SetPrototypeRangeMap", new Class[] { RangeMap.class } ) );
    context.AddConstructionTask( new ObjectArgument( MasterRangeMapIndex.Instance().GetRangeMap( context.GetCharBuffer() ) ) );
    context.AddConstructionTask( new EndMethodCallSentinel() );

  }

}
