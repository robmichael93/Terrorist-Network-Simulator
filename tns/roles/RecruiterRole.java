 ...
 * mission.  Recruiters work with trainers to turn recruits into those operators.
 * @author  Rob Michael and Zac Staples
 */
public class RecruiterRole implements TNSRole, ConnectorChangeListener {
     ...
     */
    private Color roleBackgroundColor = new Color(21, 156, 230);
    /**
    ...
    /** 
     * Creates a new instance of Recruiter 
     * @param agent The agent that owns this role.
     */
    public RecruiterRole(Agent agent) {
        this.agent = agent;
        name = new String("RecruiterRole");
//        System.out.println(agent.getAgentName() + "'s " + name + " created.");
        goals = new Vector();
//        System.out.println("Adding goals...");
        addNewGoal(new FindContactsGoal(this));
        addNewGoal(new RecruitVerificationGoal(this));
        addNewGoal(new SendRecruitsToTrainingGoal(this));
        sensors = new Vector();
//        System.out.println("Adding sensors...");
        addSensor(new ContactSensor(25.0));
//        addSensor(new LeaderSensor(25.0));
...
//        System.out.println("Adding connectors...");
        connectors.add(new FindContactConnector(this));
        tickets = new Vector();
        tickets.add(new FindContactsTicket(this));
    }
    
    /**
     * Method for hearing a connector.  The recruiter can hear Prove Allegiance, 
     * Get Trained, and Find Recruiter connectors.
    ...
    public void ConnectorChanged(ConnectorChangeEvent cce) {
        ...
        if (c.isExtended()) {
            // only detect ProveAllegianceConnectors if active goal is
            // RecruitVerification
            if (c instanceof ProveAllegianceConnector &&
                agent.getActiveGoal() instanceof RecruitVerificationGoal) {
//                System.out.println(agent.getAgentName() + " detected ProveAllegianceConnector.");
                // Add an Allegiance point to the Agent that just extended the
                // connector
                ...
                Ticket recruitVerification = new RecruitVerificationTicket(recruit);
                recruitVerification.execute();
            } else if (c instanceof GetTrainedConnector &&
                agent.getActiveGoal() instanceof SendRecruitsToTrainingGoal) {
//                System.out.println(agent.getAgentName() + " detected GetTrainedConnector.");
                Role recruit = c.getRole();
                Agent recruitAgent = ((TNSRole)recruit).getAgent();
                AgentPair pair = new AgentPair(agent, recruitAgent);
                ((TerroristAgent)agent).changeHistory(pair, 5);
                Ticket sendRecruitsToTraining = new SendRecruitsToTrainingTicket(this, c);
                sendRecruitsToTraining.execute();
            } else if (c instanceof FindRecruiterConnector) {
//                System.out.println(agent.getAgentName() + " detected FindRecruiterConnector.");
                Role leader = c.getRole();
                Agent leaderAgent = ((TNSRole)leader).getAgent();
                Ticket ticket = ((FindRecruiterConnector)c).getTicket();
