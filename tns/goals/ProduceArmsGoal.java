 ...
 */
public class ProduceArmsGoal implements TNSGoal {
    ...
    /**
     * Creates a new instance of ProduceArmsGoal 
     * @param role The role that owns this goal.
     */
    public ProduceArmsGoal(Role role) {
        this.role = role;
        goalName = new String("ProduceArmsGoal");
//        System.out.println(goalName + " created.");
    ...
    public void assignCredit(SensedEnvironment pPE) {
        ...
        int currentLevel = ((ArmsDealerRole)role).getArms().getLevel();
        int neededArms = threshold - currentLevel;
        if (neededArms < 0) {
            weight = 0;
        } else {
            weight = neededArms * 10;
        } // end if-else
        Goal lastActiveGoal = ((TerroristAgent)((TNSRole)role).getAgent()).getPastActiveGoal();
        if (lastActiveGoal != null && lastActiveGoal instanceof ProvideArmsGoal && 
            !((ArmsDealerRole)role).providedArms()) {
            weight = (int) (1.5 * weight);
        } // end if
        ((ArmsDealerRole)role).resetProvidedArmsLatch();
...
//        System.out.println(goalName + " completed.");
    ...
    public void executeActiveTicket() {
        ...
        while (e.hasMoreElements()) {
            ...
            if (t instanceof ProduceArmsTicket) {
                t.execute();
            }  // end if
        } // end while
    }
