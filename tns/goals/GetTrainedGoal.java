...
/**
 * Once a recruit has been initiated or is trustworthy enough not to need 
 * initiation, the recruit needs to meet a trainer so that he can gain some 
 * base-level experience before joining a cell and participating in missions.  
 * This goal creates the behavior of a recruiter introducing the recruit to a 
 * trainer for that purpose.
 * @author  Rob Michael and Zac Staples
 */
public class GetTrainedGoal implements TNSGoal {
    ...
    /** 
     * Creates a new instance of GetTrainedGoal 
     * @param role The role that owns this goal.
     */
    public GetTrainedGoal(Role role) {
        this.role = role;
        goalName = new String("GetTrainedGoal");
//        System.out.println(goalName + " created.");
    ...
    public void assignCredit(SensedEnvironment pPE) {
        ...
        if (((TNSGoal)proveAllegiance).isCompleted() && !completed) {
            weight = 10;
        } else if (completed) {
            weight = 0;
        } else {
            ...
            if (allegiance >= threshold) {
                weight = 10;
                ((TNSGoal)proveAllegiance).markGoalComplete();
            } else {
                weight = 0;
            } // end if-else
        } // end if-else
    ...
    public void executeActiveTicket() {
        ...
        while (e.hasMoreElements()) {
            ...
            if (t instanceof ExtendConnectorTicket) {
                if (((ExtendConnectorTicket)t).getConnectorName().equalsIgnoreCase("GetTrainedConnector")) {
                    t.execute();
                } // end if
            }  // end if
        } // end while
