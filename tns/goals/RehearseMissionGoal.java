...
/**
 * Once an operator has received a target as part of joining a leader in a cell 
 * on a mission and the leader has decided the mission needs to be rehearsed, 
 * this goal allows the operator to exhibit the behavior of the agent secluding 
 * himself with the rest of his cell to practice the upcoming mission.
 * @author  Rob Michael and Zac Staples
 */
public class RehearseMissionGoal implements TNSGoal {
    ...
    /** 
     * Creates a new instance of RehearseMissionGoal 
     * @param role The role that owns this goal.
     */
    public RehearseMissionGoal(Role role) {
        this.role = role;
        goalName = new String("RehearseMissionGoal");
//        System.out.println(goalName + " created.");
    ...
    public void assignCredit(SensedEnvironment sensedEnvironment) {
        ...
        if (((TNSGoal)joinLeader).isCompleted() && !completed) {
            int requiredRehearseTime = 0;
            int neededRehearseTime = 0;
            Target target = ((OperatorRole)role).getCurrentTarget();
            if (target != null) {
                requiredRehearseTime = target.getRehearseTime();
                neededRehearseTime = requiredRehearseTime - ((OperatorRole)role).getRehearseTurns();
                weight = neededRehearseTime * 10;
            } else {
                weight = 0;
            } // end if-else
        } else {
