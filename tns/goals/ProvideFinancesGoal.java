 ...
 */
public class ProvideFinancesGoal implements TNSGoal {
    ...
    /** 
     * Creates a new instance of ProvideFinancesGoal 
     * @param role The role that owns this goal.
     */
    public ProvideFinancesGoal(Role role) {
        this.role = role;
        goalName = new String("ProvideFinancesGoal");
//        System.out.println(goalName + " created.");
    ...
    public void assignCredit(SensedEnvironment pPE) {
        ...
        int minimumFinancesLevel = (int) (0.1 * influence);
        int currentFinancesLevel = ((FinancierRole)role).getFinances().getLevel();
        if (currentFinancesLevel <= minimumFinancesLevel) {
            weight = 0;
        } else {
            weight = (currentFinancesLevel - minimumFinancesLevel) * 10;
        } // end if-else
        if (((FinancierRole)role).financesRequested() && currentFinancesLevel > 0) {
            weight += ((FinancierRole)role).getRequestWeight();
        } // end if
        ((FinancierRole)role).resetFinancesRequestedLatch();
        ((FinancierRole)role).resetRequestWeight();
