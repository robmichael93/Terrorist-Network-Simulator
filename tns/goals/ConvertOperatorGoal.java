...
/**
 * This goal allows the leader to turn to the most influential operator in his 
 * mission and promote him to a specialist role.  The operator ceases to exist 
 * in that role, removing himself from the mission as an operator and takes on 
 * the responsibilities of creating the resources for the job he was promoted 
 * to.  When the operator leaves the mission, the leader will need to find a 
 * replacement for him, so the get operators for a mission behavior eventually 
 * re-emerges so that the leader can fill that vacancy.
 * @author  Rob Michael and Zac Staples
 */
public class ConvertOperatorGoal implements TNSGoal {
    ...
    /** 
     * Creates a new instance of ConvertOperatorGoal 
     * @param role The role that owns this goal.
     */
    public ConvertOperatorGoal(Role role) {
        this.role = role;
        goalName = new String("ConvertOperatorGoal");
//        System.out.println(goalName + " created.");
    ...
    public void assignCredit(SensedEnvironment pPE) {
        int stuckCounter = ((LeaderRole)role).getStuckCounter();
        int stuckThreshold = ((LeaderRole)role).getStuckThreshold();
        if (stuckCounter > stuckThreshold) {
            weight = stuckCounter * 10;
        } else {
            weight = 0;
        } // end if-else
...
//        System.out.println(goalName + " completed.");
    ...
    public void executeActiveTicket() {
        Ticket convertOperator = new ConvertOperatorTicket(role);
        convertOperator.execute();
    }
