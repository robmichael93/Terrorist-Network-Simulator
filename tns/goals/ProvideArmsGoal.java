 ...
 */
public class ProvideArmsGoal implements TNSGoal {
    ...
    /** 
     * Creates a new instance of ProvideArmsGoal 
     * @param role The role that owns this goal.
     */
    public ProvideArmsGoal(Role role) {
        this.role = role;
        goalName = new String("ProvideArmsGoal");
//        System.out.println(goalName + " created.");
        relationshipName = new String("ArmsBarteringRelationship");
        weight = 0;
        completed = false;
    ...
    public void assignCredit(SensedEnvironment pPE) {
        ...
        int minimumArmsLevel = (int) (0.1 * influence);
        int currentArmsLevel = ((ArmsDealerRole)role).getArms().getLevel();
        if (currentArmsLevel <= minimumArmsLevel) {
            weight = 0;
        } else {
            weight = (currentArmsLevel - minimumArmsLevel) * 10;
        } // end if-else
        if (((ArmsDealerRole)role).armsRequested() && currentArmsLevel > 0) {
            weight += ((ArmsDealerRole)role).getRequestWeight();
        } // end if
        ((ArmsDealerRole)role).resetArmsRequestedLatch();
        ((ArmsDealerRole)role).resetRequestWeight();
