 ...
 */
public class ProduceFinancesGoal implements TNSGoal {
    ...
    /** 
     * Creates a new instance of ProduceFinancesGoal 
     * @param role The role that owns this goal.
     */
    public ProduceFinancesGoal(Role role) {
        this.role = role;
        goalName = new String("ProduceFinancesGoal");
//        System.out.println(goalName + " created.");
    ...
    public void assignCredit(SensedEnvironment pPE) {
        ...
        int currentLevel = ((FinancierRole)role).getFinances().getLevel();
        int neededFinances = threshold - currentLevel;
        if (neededFinances < 0) {
            weight = 0;
        } else {
        ...
        if (lastActiveGoal != null && lastActiveGoal instanceof ProvideFinancesGoal && 
            !((FinancierRole)role).providedFinances()) {
            weight = (int) (1.5 * weight);
        } // end if
        ((FinancierRole)role).resetProvidedFinancesLatch();
...
//        System.out.println(goalName + " completed.");
    ...
    public void executeActiveTicket() {
        ...
        while (e.hasMoreElements()) {
            ...
            if (t instanceof ProduceFinancesTicket) {
                t.execute();
            }  // end if
        } // end while
    }
